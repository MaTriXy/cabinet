...
| T25 | Terminal-mode Continue uses provider-native resume — daemon persists `session.structuredOutput.sessionId` via `writeSession(codecBlob: { resumeId })` on PTY exit (Claude today; Cursor/OpenCode capture still behind stream-parsing wiring). Runner threads `adapterSessionId` into `createDaemonSession`; `buildSessionInvocation` / `buildOneShotInvocation` receive `opts.resumeId` and append `--resume` (Claude / Cursor) or `--session` (OpenCode). UI composer below a finished terminal task shows emerald "resumes in the same <provider> session" when supported; amber CircleAlert "Continue will start a fresh session" for Codex/Gemini/Grok/Copilot/Pi. | ✅ Done | `847c6e0` |
